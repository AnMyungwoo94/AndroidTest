...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (i in 1..30) {
            if (i % 2 == 0) {
                dataList.add(DataVO("홍길동${i}", "${20+i}세", "aaaa${i}@nate.com", R.drawable.ironman))
            } else {
                dataList.add(DataVO("홍길녀${i}", "${20+i}세", "aaaa${i}@nate.com", R.drawable.wonderwoman))
            }
        }
    ...
    override fun onClick(v: View?) {
       when(v?.id){
           R.id.btnFAB ->{
               var dialog = CustomDialog(this, binding)
               dialog.showDialog()
           }
       }
    }
    fun refreshRecyclerView(dataVO: DataVO){
        ...
        Toast.makeText(applicationContext, "${dataVO.tvName} 추가 되었습니다.", Toast.LENGTH_SHORT).show()
    }
}